package com.ispengya.shortlink.gateway.filter;

import com.alibaba.fastjson2.JSON;
import com.alibaba.fastjson2.JSONObject;
import com.ispengya.shortlink.gateway.config.Config;
import com.ispengya.shortlink.gateway.dto.GatewayErrorResult;
    ...
    public GatewayFilter apply(Config config) {
        return (exchange, chain) -> {
            ...
            if (!isPathInWhiteList(requestPath, requestMethod, config.getWhitePathList())) {
                ...
                Object userInfo;
                if (StringUtils.hasText(username) && StringUtils.hasText(token) && (userInfo = stringRedisTemplate.opsForHash().get("short-link:admin:token:" + username, token)) != null) {
