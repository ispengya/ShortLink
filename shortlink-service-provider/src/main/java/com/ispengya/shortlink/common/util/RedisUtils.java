...
// =======================================String=============================
    ...
    public static Long sSetStr(String key, String... values) {
        try {
            String[] s = new String[values.length];
            for (int i = 0; i < values.length; i++) {
                s[i] = values[i];
    ...
    public static Set<TypedTuple<String>> zRangeByScoreWithScores(String key,
                                                                  Double min, Double max, Long count) {
        if (Objects.isNull(min)) {
            min = Double.MIN_VALUE;
        }
        if (Objects.isNull(max)) {
        ...
        return stringRedisTemplate.opsForZSet().rangeByScoreWithScores(key, min, max, 0 ,count);
    }
